...
function result() {
  ...
  const regex = /:insertx:/gi;
  const regexy = /:inserty:/gi;
  const regexz = /:insertz:/gi;
  newStory = newStory.replace(regex, xItem);
  newStory = newStory.replace(regexy, xItem);
  newStory = newStory.replace(regexz, xItem);
  ...
  story.insertAdjacentHTML("afterend", newStory);
}
